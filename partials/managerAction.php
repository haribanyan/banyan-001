...
if (isset($_POST['action_type']) && !empty($_POST['action_type'])) {
    if ($_POST['action_type'] == 'data') {
        $conditions['where'] = array('manager_id' => $_POST['id']);
        $conditions['return_type'] = 'single';
        $user = $db->getRows($tblName, $conditions);
        echo json_encode($user);
    } elseif ($_POST['action_type'] == 'view') {
        $users = $db->getRows($tblName, array('order_by' => 'manager_id DESC'));
        if (!empty($users)) {
            $count = 0;
...
echo '<td>' . $user['emp_phoneno'] . '</td>';
		...
		if (!empty($user)) {
            ...
            $tblName = 'tbl_emp';
            $conditions['where'] = array('order_by' => 'manager_id');
...
$con = mysqli_connect("localhost","root","","real");
  $sql = mysqli_num_rows(mysqli_query($con,"SELECT * FROM tbl_emp_att where  `absent_list`='0' and `att_date`='$date' order by 1 desc limit 1")); 
  $count=$sql;
     ...
     if ($count > 0) {
        ...
        if (!empty($_POST['id'])) {
            $conditions = array('manager_id' => $_POST['id']);
...
$count=mysqli_num_rows(mysqli_query($conn,"select division_id from tbl_manager where division_id=$division and status=1"));
if($count==0){
        $userData = array(
...
'emp_pincode' => $_POST['pincode'],
            ...
            $condition = array('manager_id' => $_POST['id']);
...
$count=mysqli_fetch_array(mysqli_query($conn,"select *,tbl_division.division_name from tbl_manager INNER JOIN tbl_division
ON tbl_division.division_id = tbl_manager.division_id where tbl_division.division_id=$division and tbl_division.status=1"));
