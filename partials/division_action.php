...
if (isset($_POST['action_type']) && !empty($_POST['action_type'])) {
    if ($_POST['action_type'] == 'data') {
        $conditions['where'] = array('division_id' => $_POST['id']);
        $conditions['return_type'] = 'single';
        $user = $db->getRows($tblName, $conditions);
        echo json_encode($user);
    } elseif ($_POST['action_type'] == 'view') {
        $users = $db->getRows($tblName, array('order_by' => 'division_id DESC'));
        if (!empty($users)) {
            $count = 0;
            foreach ($users as $user): $count++;
                ...
                echo '<td>';
                echo '<a href="javascript:void(0);" onclick="viewuser(' . $cid . ')">' . $user['division_name'] . '</td>';
                echo '<td>' . $user['division_acres'] . '</td>';
                echo '<td>';
  ...
  $count=$sql;
     ...
     if ($count > 0) {
        ...
        $userData = array(
            ...
            'division_acres' => $_POST['acres'],
            'division_createdon' => date('Y-m-d'),
            'status' => 1
        );
...
echo ($i % 3 == 0) ? '<br />' : '';
        ...
        if (!empty($_POST['id'])) {
            $userData = array(
                'division_name' => $_POST['division'],
                'division_acres' => $_POST['acres'],
                'division_updatedon' => date('Y-m-d'),
                
            );
            $condition = array('division_id' => $_POST['id']);
